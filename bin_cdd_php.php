...
if ($isCli && isset($argv) && count($argv) > 0 && basename(__FILE__) === basename($argv[0])) {
    ...
    // Alias from_openapi to_docs_json -> to_docs_json
    if ($command === 'from_openapi' && isset($argv[2]) && $argv[2] === 'to_docs_json') {
        $command = 'to_docs_json';
        array_splice($argv, 1, 1); // remove from_openapi
        $argc--;
    }
    ...
    if ($command === '--help' || $command === '-h' || empty($command)) {
        ...
        echo "  --help\n";
        echo "  --version\n";
        echo "  to_openapi -f path/to/code -o spec.json\n";
        echo "  serve_json_rpc --port 8082 --listen 0.0.0.0\n";
        echo "  to_docs_json --no-imports --no-wrapping -i spec.json -o docs.json\n";
        echo "  from_openapi to_sdk_cli -i spec.json -o target_directory\n";
        echo "  from_openapi to_sdk_cli --input-dir ./specs/ -o target_directory\n";
        echo "  from_openapi to_sdk -i spec.json -o target_directory\n";
        echo "  from_openapi to_sdk --input-dir ./specs/ -o target_directory\n";
        echo "  from_openapi to_server -i spec.json -o target_directory\n";
        echo "  from_openapi to_server --input-dir ./specs/ -o target_directory\n";
        echo "\nOptions for from_openapi:\n";
        echo "  --no-github-actions\n";
        echo "  --no-installable-package\n";
        echo "\nAdditional Commands:\n";
        echo "  sync -d directory\n";
        echo "  test\n";
        exit(0);
    }
        ...
        for ($i = 2; $i < $argc; $i++) {
            if ($argv[$i] === "--port" && isset($argv[$i+1])) {
                $port = (int)$argv[++$i];
            } elseif ($argv[$i] === "--listen" && isset($argv[$i+1])) {
                $listen = $argv[++$i];
            }
        }
...
" . $resBody;
    ...
    if ($command === 'sync') {
        ...
        if (file_exists("$dir/api_cli.php") && function_exists('\Cdd\Cli\parse')) {
            $cliOps = \Cdd\Cli\parse(file_get_contents("$dir/api_cli.php"));
    ...
    if ($command === 'to_openapi' || $command === 'parse') {
        ...
        for ($i = 2; $i < $argc; $i++) {
            if ($argv[$i] === '-f' && isset($argv[$i+1])) {
                ...
                $i++;
            } else if ($argv[$i] === '-o' && isset($argv[$i+1])) {
                $outFile = $resolvePath($argv[$i+1]);
                $i++;
            } else if ($argv[$i] !== '-f' && $argv[$i] !== '-o' && $file === '') {
        ...
        if ($file !== '') {
            ...
            if ($subcommand === 'to_sdk_cli') {
                ...
                file_put_contents("$dir/api_cli.php", $cliCode);
            }
        } elseif ($inputDir !== '') {
            if (!is_dir($inputDir)) {
