...
The integration tests for this toolchain currently fail when running natively against the Stoplight Prism mock server (`./local-test.sh only-test` with `ONLY_TEST=cdd-php`).

## Instructions
1. Run the test locally for this specific language using:
   `cd .. && ONLY_TEST=cdd-php ./local-test.sh only-test`
...
4. Verify your changes by ensuring `cd .. && ONLY_TEST=cdd-php ./local-test.sh only-test` passes successfully (including the coverage validation and chaos saboteur audits).
...
The PHP SDK fails the Coverage Validation stage due to untested endpoints (e.g., `GET /v2/pet/findByTags`, `POST /v2/user/createWithList`). Generate the missing test cases.
