...
function emit_class(array $paths, string $existingCode = '', array $securityDefinitions = []): string
{
    ...
    if (strpos($out, 'public function connect_mcp(') === false) {
        ...
        $mcpConnectCode .= "            \$rpcCall = function(\$method, \$params = [], ?callable \$onNotification = null, bool \$isNotification = false, ?callable \$onSampling = null) use (&\$pipes) {\n";
        $mcpConnectCode .= "                \$id = uniqid();\n";
        $mcpConnectCode .= "                if (\$onNotification) \$params['_meta'] = ['progressToken' => \$id];\n";
        $mcpConnectCode .= "                \$reqData = ['jsonrpc' => '2.0', 'method' => \$method, 'params' => \$params];\n";
        $mcpConnectCode .= "                if (!\$isNotification) \$reqData['id'] = \$id;\n";
        $mcpConnectCode .= "                \$req = json_encode(\$reqData) . \"\\n\";\n";
        $mcpConnectCode .= "                fwrite(\$pipes[0], \$req);\n";
        $mcpConnectCode .= "                if (\$isNotification) return null;\n";
        $mcpConnectCode .= "                while ((\$line = fgets(\$pipes[1])) !== false) {\n";
        $mcpConnectCode .= "                    \$res = json_decode(\$line, true);\n";
        $mcpConnectCode .= "                    if (\$res && isset(\$res['id']) && \$res['id'] === \$id && (isset(\$res['result']) || isset(\$res['error']))) return \$res;\n";
        $mcpConnectCode .= "                    if (\$res && isset(\$res['method']) && \$res['method'] === 'sampling/createMessage') {\n";
        $mcpConnectCode .= "                        if (\$onSampling) {\n";
        $mcpConnectCode .= "                            \$sampleRes = \$onSampling(\$res['params']);\n";
        $mcpConnectCode .= "                            fwrite(\$pipes[0], json_encode(['jsonrpc' => '2.0', 'id' => \$res['id'], 'result' => \$sampleRes]) . \"\\n\");\n";
        $mcpConnectCode .= "                        } else {\n";
        $mcpConnectCode .= "                            fwrite(\$pipes[0], json_encode(['jsonrpc' => '2.0', 'id' => \$res['id'], 'error' => ['code' => -32601, 'message' => 'Sampling not supported']]) . \"\\n\");\n";
        $mcpConnectCode .= "                        }\n";
        $mcpConnectCode .= "                        continue;\n";
        $mcpConnectCode .= "                    }\n";
        $mcpConnectCode .= "                    if (\$res && !isset(\$res['id']) && isset(\$res['method']) && \$onNotification) \$onNotification(\$res['method'], \$res['params'] ?? []);\n";
        $mcpConnectCode .= "                }\n";
        ...
        $mcpConnectCode .= "        \$rpcCall('initialized', [], null, true);\n";
        ...
        $mcpConnectCode .= "            }\n";
        $mcpConnectCode .= "            public function cancel_request(string \$requestId, string \$reason = '') {\n";
        $mcpConnectCode .= "                \$call = \$this->rpcCall;\n";
        $mcpConnectCode .= "                \$call('notifications/cancelled', ['requestId' => \$requestId, 'reason' => \$reason], null, true);\n";
        $mcpConnectCode .= "            }\n";
        $mcpConnectCode .= "            public function get_tools() {\n";
        $mcpConnectCode .= "                \$call = \$this->rpcCall;\n";
        $mcpConnectCode .= "                \$tools = [];\n";
        $mcpConnectCode .= "                \$cursor = null;\n";
        $mcpConnectCode .= "                do {\n";
        $mcpConnectCode .= "                    \$res = \$call('tools/list', \$cursor ? ['cursor' => \$cursor] : []);\n";
        $mcpConnectCode .= "                    if (isset(\$res['result']['tools'])) \$tools = array_merge(\$tools, \$res['result']['tools']);\n";
        $mcpConnectCode .= "                    \$cursor = \$res['result']['nextCursor'] ?? null;\n";
        $mcpConnectCode .= "                } while (\$cursor);\n";
        $mcpConnectCode .= "                return \$tools;\n";
        $mcpConnectCode .= "            }\n";
        $mcpConnectCode .= "            public function execute_tool(string \$name, array \$args, ?callable \$onNotification = null, ?callable \$onSampling = null) {\n";
        $mcpConnectCode .= "                \$call = \$this->rpcCall;\n";
        $mcpConnectCode .= "                \$res = \$call('tools/call', ['name' => \$name, 'arguments' => \$args], \$onNotification, false, \$onSampling);\n";
        $mcpConnectCode .= "                if (isset(\$res['result']['isError']) && \$res['result']['isError'] === true) {\n";
        $mcpConnectCode .= "                     \$msg = \$res['result']['content'][0]['text'] ?? 'Tool execution returned an error state (e.g., User Denied).';\n";
        $mcpConnectCode .= "                     throw new \Exception(\$msg);\n";
        $mcpConnectCode .= "                }\n";
        ...
        $mcpConnectCode .= "                \$call = \$this->rpcCall;\n";
        $mcpConnectCode .= "                \$resources = [];\n";
        $mcpConnectCode .= "                \$cursor = null;\n";
        $mcpConnectCode .= "                do {\n";
        $mcpConnectCode .= "                    \$res = \$call('resources/list', \$cursor ? ['cursor' => \$cursor] : []);\n";
        $mcpConnectCode .= "                    if (isset(\$res['result']['resources'])) \$resources = array_merge(\$resources, \$res['result']['resources']);\n";
        $mcpConnectCode .= "                    \$cursor = \$res['result']['nextCursor'] ?? null;\n";
        $mcpConnectCode .= "                } while (\$cursor);\n";
        $mcpConnectCode .= "                return \$resources;\n";
        $mcpConnectCode .= "            }\n";
        ...
        $mcpConnectCode .= "        foreach (\$this->defaultHeaders as \$h) \$headers[] = \$h;\n";
        $mcpConnectCode .= "        foreach (\$this->bearerTokens as \$token) \$headers[] = 'Authorization: Bearer ' . \$token;\n";
        $mcpConnectCode .= "        foreach (\$this->apiKeys as \$name => \$key) \$headers[] = \$name . ': ' . \$key;\n";
